...
func (e *Engine) seriesCost(seriesKey, field string, tmin, tmax int64) query.IteratorCost {
	...
	cacheValues := e.Cache.Values(key)
	min := sort.Search(len(cacheValues), func(i int) bool {
		return cacheValues[i].UnixNano() >= tmin
	})
	// If there is no point that is greater than the minimum, then there
	// are no points in this range.
	if min == -1 {
		return c
	}

	// Search for the first point at the end of this range. If this doesn't exist,
	// then every value is within the range.
	max := sort.Search(len(cacheValues), func(i int) bool {
		return cacheValues[i].UnixNano() > tmax
	})
	if max == -1 {
		max = len(cacheValues)
	}
	c.CachedValues = int64(max - min)
	return c
}
